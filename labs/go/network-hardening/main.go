...
func main() {
	...
	fmt.Println()
	fmt.Println("1. Deny-by-default firewall rules for inbound and outbound traffic.")
	fmt.Println("2. Unique managed credentials and MFA for privileged database access.")
	fmt.Println("3. Unique employee accounts with a strong password policy.")
	fmt.Println()
	fmt.Println("## Recommendations")
	fmt.Println()
	fmt.Println("1. Prioritize firewall filtering and review the rules monthly and after network changes. This reduces the exposed attack surface and limits unauthorized traffic.")
	fmt.Println("2. Remove the default database password and require MFA for privileged access. Do this immediately, then rotate and audit the credentials regularly.")
	fmt.Println()
